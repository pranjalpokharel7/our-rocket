...
public:
  ...
  constexpr void operator=(Vec2 vec) const {
    vec.x = x;
    vec.y = y;
  }
  ...
  constexpr Vec2 operator*(const Vec2 &vec) const {
    // dot product, no cross product as they are on the same complex plane
    return Vec2(x * vec.x, y * vec.y);
  }
  constexpr bool operator==(const Vec2 &vec) const {
    return x == vec.x && y == vec.y;
  }
  constexpr bool operator!=(const Vec2 &vec) const { return !(*this == vec); }
};
...
public:
  ...
  constexpr float norm() const { return NewtonSqrt(r * r + g * g + b * b); }
  ...
  constexpr bool operator!=(const Vec3 &vec) const { return !(*this == vec); }
  constexpr Vec3 operator*(const Vec3 &vec) const {
  ...
  // Reflection refraction bla bla could also be implement but not required for
  // openGL .. glsl handles those things
};

// Implement mulitplication of vector with floating, double and integers
// Use SFINAE here
// Disables the template if condition not satisfied... Concepts could have also
// been used here but required c++20 compiler
...
// vec2
template <typename T> std::ostream &operator<<(std::ostream &os, Vec2<T> vec) {
