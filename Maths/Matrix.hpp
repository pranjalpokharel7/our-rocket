...
public:
  ...
  T *operator[](const unsigned i) { return &elements[i][0]; }

  const T *operator[](const unsigned i) const { return &elements[i][0]; }
  ...
  T *value_ptr() { return &elements[0][0]; }
};

inline Mat4<float> perspectiveProjection(float aspect_ratio, float fovy = 45.0f,
...
inline Mat4<float> lookAtMatrix(const Vec3<float> &cameraPos,
